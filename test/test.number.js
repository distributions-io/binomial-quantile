...
describe( 'number quantile', function tests() {
	...
	it( 'should evaluate the quantile function of the Binomial distribution', function test() {
		var actual;
		for ( var i = 0; i < data.length; i++ ) {
			actual =  quantile( data[ i ], n, p  );
	...
	it( 'should return `NaN` if provided `NaN` as input', function test() {
		assert.isTrue( isnan( quantile( NaN, n, p  ) ) );
	});

	it( 'should return `NaN` if provided a number outside [0,1]', function test() {
		assert.isTrue( isnan( quantile( 1.1, n, p  ) ) );
		assert.isTrue( isnan( quantile( -0.1, n, p  ) ) );
	});
