...
describe( 'matrix quantile', function tests() {
	...
	it( 'should throw an error if provided unequal length matrices', function test() {
		...
		function badValues() {
			quantile( matrix( [10,10] ), mat, n, p  );
		}
	});

	it( 'should evaluate the quantile function of the Binomial distribution for each matrix element', function test() {
		var actual, i;

		actual = matrix( [5,5], 'float64' );
		actual = quantile( actual, mat, n, p  );
	...
	it( 'should return an empty matrix if provided an empty matrix', function test() {
		...
		mat = matrix( [0,10] );
		assert.deepEqual( quantile( out, mat, n, p  ).data, expected );
