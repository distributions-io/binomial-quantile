...
function quantile( p, options ) {
	...
	if ( isNumber( p ) ) {
		return quantile1( p, opts.n, opts.p  );
	}
	if ( isMatrixLike( p ) ) {
		if ( opts.copy !== false ) {
		...
		return quantile5( out, p, opts.n, opts.p  );
	}
	if ( isTypedArrayLike( p ) ) {
		...
		return quantile6( out, p, opts.n, opts.p  );
	}
	if ( isArrayLike( p ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return quantile4( p, opts.n, opts.p , opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return quantile3( out, p, opts.n, opts.p , opts.accessor );
		}
		return quantile2( out, p, opts.n, opts.p  );
	}
	return NaN;
